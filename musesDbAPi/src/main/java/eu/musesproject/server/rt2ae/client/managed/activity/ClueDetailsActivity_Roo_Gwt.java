...
import eu.musesproject.server.rt2ae.client.managed.ui.ClueDetailsView.Delegate;
...
public abstract class ClueDetailsActivity_Roo_Gwt extends AbstractActivity implements Delegate, IsScaffoldMobileActivity {

    protected ApplicationRequestFactory requests;

    protected EntityProxyId<ClueProxy> proxyId;

    protected PlaceController placeController;

    protected AcceptsOneWidget display;

    protected ClueDetailsView view;

    public void deleteClicked() {
        if (!view.confirm("Really delete this entry? You cannot undo this change.")) {
            return;
        }
        requests.clueRequest().remove().using(view.getValue()).fire(new Receiver<Void>() {
    ...
    protected void find(Receiver<EntityProxy> callback) {
        requests.find(proxyId).with("threatId").fire(callback);
    }
}
