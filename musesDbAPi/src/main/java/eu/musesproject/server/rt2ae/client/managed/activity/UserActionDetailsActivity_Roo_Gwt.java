...
import eu.musesproject.server.rt2ae.client.managed.ui.UserActionDetailsView.Delegate;
...
public abstract class UserActionDetailsActivity_Roo_Gwt extends AbstractActivity implements Delegate, IsScaffoldMobileActivity {

    protected ApplicationRequestFactory requests;

    protected EntityProxyId<UserActionProxy> proxyId;

    protected PlaceController placeController;

    protected AcceptsOneWidget display;

    protected UserActionDetailsView view;

    public void deleteClicked() {
        if (!view.confirm("Really delete this entry? You cannot undo this change.")) {
            return;
        }
        requests.userActionRequest().remove().using(view.getValue()).fire(new Receiver<Void>() {
    ...
    protected void find(Receiver<EntityProxy> callback) {
        requests.find(proxyId).with("accessrequests").fire(callback);
    }
}
