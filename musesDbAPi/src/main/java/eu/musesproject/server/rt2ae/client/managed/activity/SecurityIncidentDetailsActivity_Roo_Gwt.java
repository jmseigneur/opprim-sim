...
import eu.musesproject.server.rt2ae.client.managed.ui.SecurityIncidentDetailsView.Delegate;
...
public abstract class SecurityIncidentDetailsActivity_Roo_Gwt extends AbstractActivity implements Delegate, IsScaffoldMobileActivity {

    protected ApplicationRequestFactory requests;

    protected EntityProxyId<SecurityIncidentProxy> proxyId;

    protected PlaceController placeController;

    protected AcceptsOneWidget display;

    protected SecurityIncidentDetailsView view;

    public void deleteClicked() {
        if (!view.confirm("Really delete this entry? You cannot undo this change.")) {
            return;
        }
        requests.securityIncidentRequest().remove().using(view.getValue()).fire(new Receiver<Void>() {
    ...
    protected void find(Receiver<EntityProxy> callback) {
        requests.find(proxyId).with("assetsId", "devicesId", "usersId").fire(callback);
    }
}
