...
import eu.musesproject.server.rt2ae.client.managed.ui.RiskinformationDetailsView.Delegate;
...
public abstract class RiskinformationDetailsActivity_Roo_Gwt extends AbstractActivity implements Delegate, IsScaffoldMobileActivity {

    protected ApplicationRequestFactory requests;

    protected EntityProxyId<RiskinformationProxy> proxyId;

    protected PlaceController placeController;

    protected AcceptsOneWidget display;

    protected RiskinformationDetailsView view;

    public void deleteClicked() {
        if (!view.confirm("Really delete this entry? You cannot undo this change.")) {
            return;
        }
        requests.riskinformationRequest().remove().using(view.getValue()).fire(new Receiver<Void>() {
    ...
    protected void find(Receiver<EntityProxy> callback) {
        requests.find(proxyId).with("accessrequestId", "assetId").fire(callback);
    }
}
