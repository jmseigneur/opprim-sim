...
import eu.musesproject.server.rt2ae.client.managed.ui.RisktreatmentDetailsView.Delegate;
...
public abstract class RisktreatmentDetailsActivity_Roo_Gwt extends AbstractActivity implements Delegate, IsScaffoldMobileActivity {

    protected ApplicationRequestFactory requests;

    protected EntityProxyId<RisktreatmentProxy> proxyId;

    protected PlaceController placeController;

    protected AcceptsOneWidget display;

    protected RisktreatmentDetailsView view;

    public void deleteClicked() {
        if (!view.confirm("Really delete this entry? You cannot undo this change.")) {
            return;
        }
        requests.risktreatmentRequest().remove().using(view.getValue()).fire(new Receiver<Void>() {
    ...
    protected void find(Receiver<EntityProxy> callback) {
        requests.find(proxyId).with("riskcommunicationId").fire(callback);
    }
}
