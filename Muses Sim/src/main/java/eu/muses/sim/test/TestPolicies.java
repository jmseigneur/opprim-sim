...
public class TestPolicies {
	...
	public static void main(String[] args) {
		...
		// First step is to configure the assets with their value with a method
		// transfroming it into sensitivity level
		Asset patentMaterial = new Asset("patent", 600000);
		SensitivityLevel.computeAssetSensitivityLevel(patentMaterial);

		SimUser user1Cso = new SimUser("user1Cso", 160, new TrustValue(0.5));
		new SimUser("user2Accountant", 80, new TrustValue(0.5));
		...
		Rule beforeTimeRule1 = new BeforeTimeRule(new Date(
				System.currentTimeMillis()));
		Rule beforeTimeRule2 = new BeforeTimeRule(new Date(
				System.currentTimeMillis()));

		CombinedRule combinedRule = new CombinedRule(beforeTimeRule1,
				beforeTimeRule2, CombinedRuleOperator.AND);

		CorporateSecurityPolicy corporateSecuritypolicy = new CorporateSecurityPolicy(
				combinedRule);
		...
		// accessRequest, //contains retrieval of threats, risks... and
		// users/devices...
		// currentLocation,
		// now,
		// currentNetworkType,
		...
		// if (deviceSecurityLevel >=
		// DeviceSecurityLevel.COMPLIANT_WITH_SECURITY_POLICY ) {
		// }
