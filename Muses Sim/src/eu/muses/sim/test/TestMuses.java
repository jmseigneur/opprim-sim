...
public class TestMuses {
    ...
    public static void main(String[] args) {
        ...
        AccessRequest accessRequest = alice.requestsAccessToAssetsForOpportunity(corporateAssets,
                aliceMustSubmitThe150000BidNow, genevaAirportGateAPublicWiFi, aliceLaptop); // aliceLaptop is for
                                                                                            // example inferred by the
                                                                                            // sensed MUSES WP6 context
        ...
        s2.generateCorporateAccessRequestRiskTreatmentsAndTrustRiskCommunication(accessRequest); // it is computed by
                                                                                                 // the MUSES WP3 RT2AE
        s2.computeCorporateAccessRequestDecision(accessRequest); // Uses threats evidence computed by the MUSES WP5
                                                                 // event correlator as well as security policies
                                                                 // administered by the MUSES WP5 event correlator but
                                                                 // the final decision is generated by the MUSES WP3
                                                                 // RT2AE
        if (!accessRequest.getCorporateAccessRequestDecision().equals(Decision.MAYBE_ACCESS)) {
            alice.readsAccessRiskCommunicationIncludingPotentialRiskTreatments(accessRequest
                    .getAccessRiskCommunication()); // including some potential other behaviours, risk treatments that
            ...
            if (alice.decidesAccessingAssetInSpiteOfRisk(accessRequest)) {
                ...
                if (alice.decidesToApplyAProposedRiskTreatment(accessRequest)) {
                    alice.movesTo(genevaAirportSecuredCorporateLoungeWiFi); // this risk treatment allows her to access
                                                                            // the asset
                    Asset[] usedCorporateAssets = alice.getCorporateAssets(accessRequest);
                    alice.usesCorporateAssets(usedCorporateAssets);
                    s2.logsBeneficialOutcomeBasedOnTheAchievedOpportunity(accessRequest);
                    ...
                    s2.logsAccessRequestUserDecisionInMusesCompanyInstance(); // It may be important to also log when a
                                                                              // user decides not taking the opportunity
                                                                              // due to risk, e.g., to avoid never
                                                                              // taking opportunities even when there is
                                                                              // no risk due to laziness or risk
                                                                              // aversion...
                }
            }
        }
        ...
        Outcome securityIncidentOnPatent = new SecurityIncident("New patent proposal comprised",
                newPatentProposal.getValue()); // the MUSES WP5 event correlator would detect this incident
