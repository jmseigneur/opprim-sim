...
public class TestMusesConfiguration {
	...
	public static void main(String[] args) {
		...
		// PublicAccessPoint genevaAirportGateAPublicWiFi = new
		// PublicAccessPoint("genevaAirportGateAPublicWiFiAP", 0.0,
		// 0.0);
		// PublicAccessPoint genevaAirportSecuredCorporateLoungeWiFi = new
		// PublicAccessPoint("genevaAirportSecureCorporateLoungeWiFiAP", 0.0,
		// 0.0);
		// user1.movesTo(genevaAirportGateAPublicWiFi);

		// "user1 has two hours to work on a marketing poster at an airport that
		// will be public
		// with public material otherwise two hours of her working time will be
		// lost"
		// "user1 must access documents for a 150 000 kEuros bid to win a new
		// project to submit
		...
		AccessRequest accessRequest = SimUser
				.requestsAccessToAsset(TestMusesConfiguration.materialForPatentProposal);
		...
		opportunityDescriptor.addOutcome(new Outcome(
				...
				150.0));
		opportunityDescriptor
				.addRequestedAsset(TestMusesConfiguration.materialForPatentProposal);
		accessRequest.setOpportunityDescriptor(opportunityDescriptor);
		accessRequest.setUser(userCso);

		// XXX //user1Laptop is for example inferred by the sensed MUSES WP6
		// context observation and their events
		// correlation with MUSES WP5

		Decision decision = TestMusesConfiguration.s2Rt2ae
				.decidesBasedOnConfiguredRiskPolicy(accessRequest);
		System.out.println("The computed decision for the asset "
				+ accessRequest.getRequestedCorporateAsset().iterator().next()
						.getAssetName()
				+ " was: "
		...
		if (!decision.equals(Decision.STRONG_DENY_ACCESS)) {
			if (!decision.equals(Decision.ALLOW_ACCESS)) {
				TestMusesConfiguration.user1
						.readsAccessRiskCommunicationIncludingPotentialRiskTreatments(decision
								.getRiskCommunication()); // including some
															// potential other
															// behaviours, risk
															// treatments that
			...
			while (decision.equals(Decision.MAYBE_ACCESS)
					&& TestMusesConfiguration.user1
							.isStillMakingRequest(accessRequest)) {
				if (TestMusesConfiguration.user1
						.givesUpRequestDueToRisk(accessRequest)) {
					TestMusesConfiguration.user1.setStillMakingRequest(
							accessRequest, false);
				} else {
					if (decision
						...
						if (TestMusesConfiguration.user1
							...
							opportunityDescriptor = TestMusesConfiguration.user1
									.refinesOpportunity(); // in our example
							// it corresponds
							// to
							// refinesOpportunity()
							// below
					...
					decision = TestMusesConfiguration.s2Rt2ae
							.decidesBasedOnConfiguredRiskPolicy(accessRequest);
					TestMusesConfiguration.user1
							.readsAccessRiskCommunicationIncludingPotentialRiskTreatments(decision
									.getRiskCommunication()); // including some
																...
																// treatments
																// that
					// would allow the user to access the asset with less risk,
					// such
			...
			while (decision.equals(Decision.ON_YOUR_RISK_ACCESS)
				...
				if (TestMusesConfiguration.user1
					...
					// It may be important to also log when a user decides not
					// taking the opportunity due to risk, e.g.,
					// to avoid never taking opportunities even when there is no
					// risk due to laziness or risk
					// aversion...
				} else {
					if (TestMusesConfiguration.user1
							.decidesAccessingAssetInSpiteOfRisk(accessRequest)) {
						TestMusesConfiguration.user1.setStillMakingRequest(
								accessRequest, false);
						Asset[] corporateAssets = TestMusesConfiguration.user1
						...
						TestMusesConfiguration.user1
								.usesCorporateAssets(corporateAssets);
						if (TestMusesConfiguration.user1
							...
							TestMusesConfiguration.s2Rt2ae
						...
						TestMusesConfiguration.user1
								.readsAccessRiskCommunicationIncludingPotentialRiskTreatments(decision
										.getRiskCommunication()); // including
																	// some
																	// potential
																	// other
																	// behaviours,
			...
			SecurityIncident securityIncidentOnPatent = new SecurityIncident(
					...
					TestMusesConfiguration.newPatentProposal.getValue()); // the
																			// MUSES
																			// WP5
																			// event
																			// correlator
			...
			// this incident
			TestMusesConfiguration.s2EventCorrelator
					.reportsSecurityIncident(securityIncidentOnPatent);
			if (TestMusesConfiguration.s2EventCorrelator
				...
				TestMusesConfiguration.s2MusesClientApp
						.warnsUserResponsibleForSecurityIncident(
								TestMusesConfiguration.user1,
								securityIncidentOnPatent);
				TestMusesConfiguration.s2Rt2ae.decreasesTrustInUser(
						TestMusesConfiguration.user1, securityIncidentOnPatent);
				s2Rt2ae.recalculateThreatProbabilitiesWhenIncident(accessRequest);
			}
		} else {
			TestMusesConfiguration.s2EventCorrelator
					.logDeniedRequest(accessRequest);
		}

		// CorporateSecurityPolicy noAccessOutsideOfCorporatePremices = new
		// CorporateSecurityPolicy();

		// noAccessOutsideOfCorporatePremices.setAcceptableRiskThreshold(Location.EVERYWHERE,
		// RiskValue.NO_RISK); //
		// Equivalent to deny access everywhere

		// CorporateOffice corporateOffice = new
		// CorporateOffice("Corporate Office", 0.0, 0.0);

		// noAccessOutsideOfCorporatePremices.setAcceptableRiskThreshold(corporateOffice,
		// RiskValue.FULL_RISK); //
	...
	public static void musesUsersDevicesAndAssetsConfigurationsSteps() {
		...
		RiskPolicy riskPolicy = TestMusesConfiguration.userCso
		...
		// with costs and
		// benefits
		TestMusesConfiguration.s2EventCorrelator = TestMusesConfiguration.userCso
				.configureEventCorrelator();
		TestMusesConfiguration.s2Rt2ae = TestMusesConfiguration.userCso
				.configureRt2ae(TestMusesConfiguration.s2EventCorrelator,
		...
		if (TestMusesConfiguration.userCso
				.hasCsoConfiguredAssets(TestMusesConfiguration.s2Rt2ae)) {
			...
			TestMusesConfiguration.materialForBid = TestMusesConfiguration.s2Rt2ae
					.getAsset("materialForBid");
			TestMusesConfiguration.newPatentProposal = TestMusesConfiguration.s2Rt2ae
			...
			TestMusesConfiguration.newPatentProposal = TestMusesConfiguration.userCso
					.addsNewAssetToRt2ae(TestMusesConfiguration.s2Rt2ae,
							new Asset("newPatentProposal", 600000));
			// Vulnerability patentInvalidationTrial = new Vulnerability();
			// Vector<Vulnerability> vulnerabilitiesNewPatent = new
			...
			TestMusesConfiguration.materialForPatentProposal = TestMusesConfiguration.userCso
					.addsNewAssetToRt2ae(
							TestMusesConfiguration.s2Rt2ae,
							new Asset("materialForPatentProposal",
									TestMusesConfiguration.newPatentProposal
											.getValue()));
			// Collection<Vulnerability> vulnerabilitiesPatentMaterial =
			// s2EventCorrelator.getVulnerabilities(materialForPatentProposal);
			// Vulnerability materialInterceptedOverTheNetwork = new
			// Vulnerability();
			...
			TestMusesConfiguration.materialForBid = TestMusesConfiguration.userCso
					.addsNewAssetToRt2ae(TestMusesConfiguration.s2Rt2ae,
							new Asset("materialForBid", 150000));
			// Collection<Vulnerability> vulnerabilitiesBidMaterial =
			// s2EventCorrelator.getVulnerabilities(materialForPatentProposal);
			// Vulnerability materialOfBidInterceptedOverTheNetwork = new
			// Vulnerability();
			...
			TestMusesConfiguration.materialForPublicMarketingPoster = TestMusesConfiguration.userCso
					.addsNewAssetToRt2ae(TestMusesConfiguration.s2Rt2ae,
							new Asset("materialForPublicMarketingPoster", 0));
		}

		TestMusesConfiguration.userCso
				.enrollsUserInCompanyInformationSystem(TestMusesConfiguration.user1);
		...
		if (TestMusesConfiguration.userCso.checksIfDeviceIsTrustworthyEnough()) {
			TestMusesConfiguration.user1Laptop
					.installOnClient(TestMusesConfiguration.s2MusesClientApp);
			String userLogin = TestMusesConfiguration.user1.getNickname();
			String password = TestMusesConfiguration.user1.typePassword();
			if (TestMusesConfiguration.user1.authenticateWihMusesCredentials(
					userLogin, password)) {
				TestMusesConfiguration.user1Laptop
						.setOwner(TestMusesConfiguration.user1.getNickname());
				TestMusesConfiguration.user1.authenticates();
				Certificate deviceKeyPairMusesCertificate = TestMusesConfiguration.s2EventCorrelator
						.logNewDevice(TestMusesConfiguration.user1Laptop
								.getCryptoId(),
	...
	public static void refinesOpportunity() {
		...
		requestedAssestsByUser1
				.add(TestMusesConfiguration.materialForPublicMarketingPoster);
		requestedAssestsByUser1
				.add(TestMusesConfiguration.materialForPatentProposal);

		new Outcome("Gained two hours work of user1",
				2 * TestMusesConfiguration.user1.getHourlyCost());
		new Outcome("Lost two hours work of user1", -1 * 2
				* TestMusesConfiguration.user1.getHourlyCost());
		new Outcome("Gained submitted bid", 150000);
		new Outcome("Lost bid", 150000);
	}

	/*
	 * public static void computesThreats(OpportunityDescriptor
	 * opportunityDescriptor) { Collection<Asset> requestedAssestsByUser1 =
	 * opportunityDescriptor.getRequestedAssets(); // "user1 has two hours to
	 * work on a marketing poster at an airport that will be public with public
	 * material otherwise two hours of her working time will be
	 * lost" Outcome gainedTwoHoursWorkOfUser1 = new Outcome("Gained two hours
	 * work of user1", 2 * user1.getHourlyCost()); Outcome
	 * lostTwoHoursWorkOfUser1 = new Outcome("Lost two hours work of user1", -1
	 * * 2 * user1.getHourlyCost()); // "user1 must access documents for a 150
	 * 000 kEuros bid to win a new project to submit it now or it will be too
	 * late because the deadline will have passed" Outcome gainedSubmittedBid =
	 * new Outcome("Gained submitted bid", 150000); Outcome lostBid = new
	 * Outcome("Lost bid", 150000); //"user1 works on a patent with confidential
	 * material Outcome materialForPatentProposalIsStolen = new
	 * Outcome("Material for patent proposal is stolen and invalidates patent",
	 * -1 * materialForPatentProposal.getValue()); Outcome
	 * newPatentProposalIsNotInvalidated = new
	 * Outcome("New patent is NOT invalidate", newPatentProposal.getValue());
	 * Vector<Outcome> outcomesUser1WorksTwoHoursAtTheAirport = new
	 * Vector<Outcome>();
	 ...
	 * Probability probabilityGainedTwoHoursWorkOfUser1 =
	 * s2Rt2ae.callEventCorrelatorForProbability(gainedTwoHoursWorkOfUser1,
	 * s2Rt2ae.getTrustValue(user1)); Probability
	 * probabilityMaterialForPatentProposalIsStolen =
	 * s2Rt2ae.callEventCorrelatorForProbability
	 * (materialForPatentProposalIsStolen, s2Rt2ae.getTrustValue(user1));
	 * Opportunity opportunityUser1WorksTwoHoursAtTheAirport = new
	 * Opportunity("user1 works two hours at the airport",
	 * probabilityGainedTwoHoursWorkOfUser1,
	 * outcomesUser1WorksTwoHoursAtTheAirport); Threat
	 * threatAttackerStealsMaterialForPatentProposal = new
	 * Threat("Attacker steals material for patent proposal",
	 * probabilityMaterialForPatentProposalIsStolen,
	 * outcomesPatentMaterialIsStolen); Threat[] newPatentProposalThreats =
	 * s2EventCorrelator.getThreats(newPatentProposal); Threat[]
	 * materialForPatentProposalThreats =
	 * s2EventCorrelator.getThreats(materialForPatentProposal); Threat[]
	 * materialForBidThreats = s2EventCorrelator.getThreats(materialForBid);
	 * Threat[] materialForPublicMarketingPosterThreats =
	 * s2EventCorrelator.getThreats(materialForPublicMarketingPoster);
	 * Vector<RiskEvent> riskEvents = new Vector<RiskEvent>();
	 * riskEvents.add(opportunityUser1WorksTwoHoursAtTheAirport);
	 * riskEvents.add(threatAttackerStealsMaterialForPatentProposal); for
	 * (RiskEvent riskEvent : newPatentProposalThreats) {
	 * riskEvents.add(threatAttackerStealsMaterialForPatentProposal); }
	 * AccessRequest accessRequest =
	 * user1.requestsAccessToAssetsForOpportunity(requestedAssestsByUser1,
	 * opportunityUser1WorksTwoHoursAtTheAirport, genevaAirportGateAPublicWiFi,
	 * user1Laptop); //user1Laptop is for example inferred by the sensed MUSES
	 * WP6 context observation and their events correlation with MUSES WP5
